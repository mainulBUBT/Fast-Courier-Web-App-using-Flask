...
class Merchant(db.Model):
    ...
    due_charge = db.Column(db.String(100))
...
class Parcel(db.Model):
    ...
    due_charge = db.Column(db.Integer)
...
class ReturnParcel(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    parcel_id = db.Column(db.Integer, db.ForeignKey('parcel.id'), nullable=False)
    reason = db.Column(db.String(150), default= 0, nullable=False)
    report_date = db.Column(db.DateTime, nullable = False, default = datetime.utcnow)
    updated_date = db.Column(db.DateTime, default=datetime.now, onupdate=datetime.utcnow)